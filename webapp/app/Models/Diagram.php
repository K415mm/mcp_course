...
class Diagram extends Model
{
    ...
    public function getKrokiUrlAttribute()
    {
        ...
        return "https://kroki.io/diagramsnet/svg/" . $urlSafeBase64;
    }

    /**
     * Get the native SVG data URI if saved in xmlsvg format, 
     * otherwise fallback to the Kroki URL.
     */
    public function getThumbnailUriAttribute()
    {
        if (!$this->xml_data) {
            return null;
        }

        // If it's a native SVG string generated by Draw.io (xmlsvg format)
        if (strpos(trim($this->xml_data), '<svg') === 0) {
            return 'data:image/svg+xml;base64,' . base64_encode($this->xml_data);
        }

        return $this->kroki_url;
    }
}
