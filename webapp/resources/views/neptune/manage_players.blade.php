@extends('layouts.app')
@section('title', 'Manage Players — ' . ($session->name ?? 'Neptune Strike'))
    ...
    <div class="row justify-content-center">
        <div class="col-lg-10">
            <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-3">
                <div>
                    <h2 class="mb-1 text-theme cs-mono"><i class="bi bi-people-fill me-2"></i>Manage Players</h2>
                    <div class="text-white-50">Session: {{ $session->name }} ({{ $session->code }})</div>
                </div>
                <div class="d-flex flex-wrap gap-2">
                    <a href="{{ route('neptune.show', $session->code) }}" class="btn btn-outline-theme fw-bold" target="_blank">
                        <i class="bi bi-person-workspace me-1"></i>Team View
                    </a>
                    <a href="{{ route('neptune.dashboard', $session->code) }}" class="btn btn-outline-secondary fw-bold" target="_blank">
                        <i class="bi bi-display me-1"></i>Grand Screen
                    </a>
                    <button onclick="copyJoinLink()" class="btn btn-outline-warning fw-bold">
                        <i class="bi bi-clipboard me-1"></i>Copy Join Link
                    </button>
                    <a href="{{ route('neptune.moderator', $session->code) }}" class="btn btn-outline-info fw-bold">
                ...
                <div class="card-body">
                    <div class="user-admin-card">
                        ...
                        <div class="text-white-50 mb-3" id="assignUsersHelp">
                            Assign verified students to roles. Recommended before start, but always possible during active sessions.
                        </div>
                        <div class="row g-3 align-items-start">
                            <div class="col-md-4">
                                <label class="form-label small text-white-50">Target Role</label>
                            ...
                            <div class="col-md-8">
                                ...
                                <div class="user-meta-line mt-1" id="assignableCount">0 students available</div>
                            </div>
                            <div class="col-12 mt-3 text-end">
                                <button class="btn btn-theme fw-bold px-4" id="assignUsersBtn" onclick="assignSelectedUsers()">Assign to Role</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        <div class="fw-bold text-theme" style="font-size: 1.1rem;"><i class="bi bi-shield-lock me-2"></i>Active Session Players</div>
                        <div class="d-flex align-items-center gap-3">
                            <div class="input-group input-group-sm" style="width: 250px;">
                                <span class="input-group-text bg-transparent border-end-0"><i class="bi bi-search"></i></span>
                                <input type="text" class="form-control border-start-0 ps-0" id="playerSearch" placeholder="Search (name, email, role)..." onkeyup="renderPlayersRoster()">
                            </div>
                            <div class="badge bg-dark fs-6">Total: <span id="usersCount">0</span></div>
                        </div>
                    </div>
                    <div class="text-white-50 mb-3">
                        Move, rename, ban, or remove players. Changes apply instantly to all views.
                    </div>
...
async function apiChecked(path, method='GET', body=null) {
    ...
    if (!r.ok || data?.ok === false || data?.success === false) {
        ...
        throw new Error(data?.error || validationMessage || 'Operation rejected.');
    }
    return data;
}
...
function renderAssignmentControls() {
    ...
    count.textContent = `${assignableUsersCache.length} students available`;
}
...
function renderPlayersRoster() {
    ...
    if (!filteredPlayers.length) {
        root.innerHTML = '<div class="text-white-50 text-center py-5">No players match your search query.</div>';
        return;
    }
                ...
                <div class="user-roster-row ${player.isBanned ? 'banned' : ''}" id="player-row-${player.id}">
                    <div class="d-flex align-items-start justify-content-between gap-2 mb-2">
                        <div>
                            ...
                            <div class="user-meta-line">${escapeHtml(player.email || 'No linked email')} · ${escapeHtml(player.teamName || 'No role')}</div>
                        </div>
...
async function assignSelectedUsers() {
    ...
    if (!teamType) {
        showNotif('Choose a target role.', 'warn');
        return;
    }
    ...
    try {
        ...
        showNotif(e.message || 'Assignment rejected.', 'danger');
    }
}
...
async function savePlayer(playerId) {
    ...
    if (!name || !teamType) {
        showNotif('Name and role are required.', 'warn');
        return;
    }
    ...
    try {
        ...
        showNotif(e.message || 'Update rejected.', 'danger');
    }
}

async function banPlayerAction(playerId) {
    const reason = await swalPrompt('Reason for banning (optional):', '') ?? '';
    try {
        await apiChecked(`players/${playerId}/ban`, 'POST', { reason: reason.trim() });
        showNotif('Player banned.', 'success');
        await poll();
    } catch (e) {
        showNotif(e.message || 'Banning rejected.', 'danger');
    }
}
...
async function unbanPlayerAction(playerId) {
    try {
        ...
        showNotif(e.message || 'Unbanning rejected.', 'danger');
    }
}

async function removePlayerAction(playerId) {
    if (!await swalConfirm('Remove this player from the session? They will need to be re-assigned.')) return;
    try {
        await apiChecked(`players/${playerId}`, 'DELETE');
        showNotif('Player removed from session.', 'success');
        await poll();
    } catch (e) {
        showNotif(e.message || 'Removal rejected.', 'danger');
    }
}
